...
#            from pyNN.spiNNaker import record
    ...
    def _interconnect_neurons(self, network, verbose=False):
        ...
        for y in range(0, self.dim_y):
            ...
            connList.append(
                (y + self.dim_y, y, synaptic_params['wBC'], synaptic_params['dBC']))
        ...
        for ensemble in network:
            ps.Projection(ensemble[0], ensemble[1], ps.FromListConnector(
                connList), receptor_type='inhibitory')
    ...
    def _interconnect_neurons_inhexc(self, network, verbose=False):
        ...
        for ensemble in network:
            ps.Projection(ensemble[1], ensemble[1], ps.FromListConnector(
                nbhoodEcxY), receptor_type='excitatory')
    ...
    def get_accumulated_disparities(self, sort_by_disparity=True, save_spikes=True):
        if sort_by_disparity:
            ...
            for d in range(0, self.max_disparity - self.min_disparity + 1):
                collector_cells = [self.network[x][1]
                                   for x in same_disparity_indices[d]]
                spikes_per_disparity_map.append(
                    sum([sum(x.get_spike_counts().values()) for x in collector_cells]))
                if save_spikes:
                    if not os.path.exists("./spikes"):
                        os.makedirs("./spikes")
                    i = 0
            ...
            all_spikes = sum(sum(x[1].get_spikes_count().values()
                             for x in self.network))
            return all_spikes
